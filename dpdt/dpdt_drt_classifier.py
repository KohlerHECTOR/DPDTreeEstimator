...
class DrTTreeClassifier(ClassifierMixin, BaseEstimator):
    """Dr. T Dynamic Programming Decision Tree (DPDTree) classifier.
    ...
    random_state : int, default=42
        Fixes randomness of the classifier. Randomness happens in the calls to kmeans.
    kmeans_centers_list : list of int, default=(32,)
        List containing the number of KMeans cluserts at each depth.
    ...
    Examples
    --------
    >>> from dpdt import DrTTreeClassifier
    >>> clf = DrTTreeClassifier()
    >>> clf.fit([[0, 0], [1, 1]], [0, 1])
    DrTTreeClassifier()
    >>> clf.predict([[2., 2.]])
    array([1])
    ...
    _parameter_constraints = {
        ...
        "kmeans_centers_list": ["array-like"],
    ...
    def __init__(
        ...
        self.kmeans_centers_list = kmeans_centers_list
    ...
    def fit(self, X, y):
        ...
        self._root = State(
            ...
            max_action_nb=self.kmeans_centers_list[0] * self.X_.shape[1] + 1,
        )
        self._count_ops = 0
    ...
    def _expand_node(self, node, depth=0):
        ...
        if rstar < 0 and depth < self.max_depth:
            valid_features = []
            valid_thresholds = []
            if depth < len(self.kmeans_centers_list):
                nb_clusters = max(
                    [2, min(self.kmeans_centers_list[depth], sum(node.nz))]
                )
            else:
                nb_clusters = 2

            for f in range(self.X_.shape[1]):
                feat = self.X_[node.nz][:, f]
                km = KMeans(
                    n_clusters=nb_clusters,
                    random_state=self.random_state,
                )
                km.fit(feat[:, np.newaxis])
                for threshold in km.cluster_centers_:
                    valid_features.append(f)
                    valid_thresholds.append(threshold[0])
            ...
            act_max = (
                self.kmeans_centers_list[depth + 1] * self.X_.shape[1]
                if depth + 1 < len(self.kmeans_centers_list)
                else 2 * self.X_.shape[1]
            )
